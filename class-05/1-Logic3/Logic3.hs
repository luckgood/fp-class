...
data Logic3 = T | U | F deriving (Show,Eq,Ord)
...
any3 p = foldr (\x flag -> if p x == T then flag \/ T else flag) F
all3 p = foldr (\x flag -> if p x == T then flag else flag /\ F) T
...
excluded_fourth = any3 (\x -> x \/ not3 x \/ not3(not3 x)) [T, U, F]
